    ...
    try {
        if (index === "all") {
            ...
            if (riveRes.status === "fulfilled" && Array.isArray(riveRes.value)) {
                streams.push(...riveRes.value);
            }
            if (webstreamerRes.status === "fulfilled") {
                streams.push(...webstreamerRes.value);
            }
            if (mappleRes.status === "fulfilled" && mappleRes.value.success && mappleRes.value.data) {
                streams.push(mappleRes.value.data);
            }
            if (cinemaOSRes.status === "fulfilled" && cinemaOSRes.value && !cinemaOSRes.value.error) {
                const cinemaOSData = cinemaOSRes.value;
                if (Array.isArray(cinemaOSData.streams)) {
                    streams.push(...cinemaOSData.streams.map(({ provider, ...rest }) => rest));
                }
            }
            if (vidrockRes.status === "fulfilled" && Array.isArray(vidrockRes.value)) {
                streams.push(...vidrockRes.value.map(({ provider, ...rest }) => rest));
            }
            if (movieboxRes.status === "fulfilled" && movieboxRes.value.success) {
                streams.push(...movieboxRes.value.streams.map(({ provider, ...rest }) => rest));
            }
            if (fuckitRes.status === "fulfilled" && fuckitRes.value.success) {
                streams.push(...fuckitRes.value.streams.map(({ provider, ...rest }) => rest));
            }
        ...
        if (idx === 1 || idx === 3 || idx === 4) {
            ...
            const data = await getMovieBoxStream(tmdbId, type, season, episode);
            return c.json(data);
        }
        else if (idx === 5) {
            ...
            const data = await getMappleStream(tmdbId, type, season, episode);
            if (data?.data?.captions) delete data.data.captions;
            return c.json(data);
        } else if (idx === 7) {
            const data = await getCinemaOSStream(tmdbId, type, season, episode);
            return c.json(data);
        } else if (idx === 8) {
            ...
            const data = await getFuckItStream(tmdbId, imdbId, title, releaseYear, type, season, episode);
            return c.json(data);
        }
        else {
            return c.json({ error: "Invalid index. Use 1, 3, 4 for Rive, 2 for MovieBox, 5 for Webstreamer, 6 for Mapple, 7 for CinemaOS, 8 for VidRock, 9 for FuckIt, or 'all'" }, 404);
        }
    } catch (error) {
